...
export default function MatrixPage() {
  ...
  return (
    <main className="page">
      ...
      {tab === "matrix" ? <>
        <div className="matrix-legend"><span><strong className="status status-assigned">Reading required</strong></span><span><strong className="status status-completed">Read and acknowledged</strong></span><span><strong className="status status-overdue">Reading overdue</strong></span><span><strong className="status status-not-assigned">No assignment</strong></span><span><strong className="status">Reference only</strong></span><span>Click any operator status to view their complete current and historical training record.</span></div>
        {!matrix?.rows.length ? <EmptyState title="No curriculum defined" detail="Add controlled documents to this role to generate its live matrix." /> : <div className="matrix-scroll"><table className="matrix-table"><thead><tr><th className="sticky-col">Controlled document</th><th>Version</th>{matrix.users.map((user) => <th key={user.id}><div className="vertical-name">{user.display_name}</div></th>)}</tr></thead><tbody>{matrix.rows.map((row) => <tr key={row.requirement.id}><td className="sticky-col"><strong>{row.requirement.document_code}</strong><span>{row.requirement.document_title}</span><small>{row.requirement.requirement_type.replaceAll("_", " ")}</small></td><td>{row.current_version?.version_label ?? "No effective version"}</td>{row.cells.map((cell) => <td key={cell.user_id} title={`${trainingStatusLabel(cell.status)}${cell.due_at ? ` · due ${formatDate(cell.due_at)}` : ""}`}><button className={`matrix-cell ${statusClass(cell.status)}`} onClick={() => { const user = matrix.users.find((item) => item.id === cell.user_id); if (user) setHistoryUser(user); }}>{matrixStatusLabel(cell.status)}</button></td>)}</tr>)}</tbody></table></div>}
