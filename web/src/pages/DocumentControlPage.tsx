...
import { ArrowRight, BookCopy, BookOpen, Check, Download, FilePlus2, FolderSearch, History, Pencil, Plus, Search, ShieldCheck } from "lucide-react";
...
type FamilyDraft = {
  code: string; title: string; document_type: string; owner_department: string; description: string;
  review_interval_months: number; is_active: boolean; reason: string;
};

const blankFamily: FamilyDraft = { code: "", title: "", document_type: "SOP", owner_department: "", description: "", review_interval_months: 24, is_active: true, reason: "" };
...
export default function DocumentControlPage() {
  ...
  const visible = documents.filter((item) => `${item.code} ${item.title}`.toLowerCase().includes(search.toLowerCase()));
    ...
    <main className="page">
      ...
      <section className="control-workspace">
        <aside className="control-list panel">
          ...
          <div className="control-list-scroll">{visible.map((item) => <button key={item.id} className={selectedId === item.id ? "active" : ""} onClick={() => setSelectedId(item.id)}><span className="doc-type">{item.document_type.slice(0, 3)}</span><div><strong>{item.code}</strong><span>{item.title}</span></div><ArrowRight /></button>)}</div>
        </aside>
        <section className="control-detail panel">
          {!detail ? <EmptyState title="Select a document" detail="Choose a master-list record to see its controlled history." /> : <>
            <div className="control-title"><div><div className="document-tags"><span>{detail.document_type}</span><span>{detail.owner_department}</span>{!detail.is_active && <span>Inactive family</span>}</div><p className="doc-code">{detail.code}</p><h2>{detail.title}</h2><p>{detail.description || "No additional description."}</p></div><div className="button-row">{has("documents.manage") && <><button className="button secondary" onClick={() => setFamilyModal("edit")}><Pencil /> Edit</button><button className="button primary" onClick={() => setVersionModal(true)}><FilePlus2 /> New revision</button></>}</div></div>
...
function TransitionButtons({ version, has, onSelect }: { version: DocumentVersion; has: (p: string) => boolean; onSelect: (action: string) => void }) {
...
function FamilyModal({ mode, document, onClose, onSaved }: { mode: "create" | "edit"; document: ControlledDocument | null; onClose: () => void; onSaved: () => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { await api(mode === "create" ? "/documents" : `/documents/${document?.id}`, { method: mode === "create" ? "POST" : "PATCH", body: JSON.stringify(draft) }); onSaved(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to save"); } finally { setBusy(false); } };
  return <Modal title={mode === "create" ? "Create controlled document" : "Edit document metadata"} onClose={onClose} wide><form className="form-grid" onSubmit={submit}><label>Document number<input value={draft.code} onChange={(e) => setDraft({ ...draft, code: e.target.value })} disabled={mode === "edit"} required /></label><label>Document type<select value={draft.document_type} onChange={(e) => setDraft({ ...draft, document_type: e.target.value })}>{documentTypes.map((item) => <option key={item}>{item}</option>)}</select></label><label className="span-2">Title<input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} required /></label><label>Owner department<input value={draft.owner_department} onChange={(e) => setDraft({ ...draft, owner_department: e.target.value })} required /></label><label>Review interval (months)<input type="number" min="1" max="120" value={draft.review_interval_months} onChange={(e) => setDraft({ ...draft, review_interval_months: Number(e.target.value) })} /></label><label className="span-2">Description<textarea value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })} /></label>{mode === "edit" && <label className="span-2 checkbox-line"><input type="checkbox" checked={draft.is_active} onChange={(event) => setDraft({ ...draft, is_active: event.target.checked })} /> Active master-list document</label>}<label className="span-2">Reason for change<textarea value={draft.reason} onChange={(e) => setDraft({ ...draft, reason: e.target.value })} required /></label><ErrorBanner error={error} /><div className="modal-actions span-2"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? "Saving…" : "Save document"}</button></div></form></Modal>;
}
...
function VersionModal({ document, onClose, onSaved }: { document: ControlledDocument; onClose: () => void; onSaved: () => void }) {
  ...
  const [browser, setBrowser] = useState(false); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { await api(`/documents/${document.id}/versions`, { method: "POST", body: JSON.stringify({ ...draft, issue_date: draft.issue_date || null, effective_at: draft.effective_at ? new Date(draft.effective_at).toISOString() : null, review_due_date: draft.review_due_date || null, training_impact_reason: draft.training_impact_reason || null }) }); onSaved(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to register revision"); } finally { setBusy(false); } };
  return <><Modal title={`Register a revision for ${document.code}`} onClose={onClose} wide><form className="form-grid" onSubmit={submit}><label>Version label<input placeholder="V01" value={draft.version_label} onChange={(e) => setDraft({ ...draft, version_label: e.target.value })} required /></label><label>Training impact<select value={draft.training_impact} onChange={(e) => setDraft({ ...draft, training_impact: e.target.value })}><option value="RETRAIN">Retraining required</option><option value="NO_RETRAIN">No retraining required</option></select></label><label className="span-2">Controlled source file<div className="input-action"><input value={draft.relative_path} readOnly required /><button type="button" className="button secondary" onClick={() => setBrowser(true)}><FolderSearch /> Browse</button></div></label><label className="span-2">Change summary<textarea value={draft.change_summary} onChange={(e) => setDraft({ ...draft, change_summary: e.target.value })} required /></label>{draft.training_impact === "NO_RETRAIN" && <label className="span-2">No-retraining rationale<textarea value={draft.training_impact_reason} onChange={(e) => setDraft({ ...draft, training_impact_reason: e.target.value })} required /></label>}<label>Issue date<input type="date" value={draft.issue_date} onChange={(e) => setDraft({ ...draft, issue_date: e.target.value })} /></label><label>Effective date/time<input type="datetime-local" value={draft.effective_at} onChange={(e) => setDraft({ ...draft, effective_at: e.target.value })} /></label><label>Review due date<input type="date" value={draft.review_due_date} onChange={(e) => setDraft({ ...draft, review_due_date: e.target.value })} /></label><label>Registration reason<input value={draft.reason} onChange={(e) => setDraft({ ...draft, reason: e.target.value })} required /></label><ErrorBanner error={error} /><div className="modal-actions span-2"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? "Verifying file…" : "Register immutable revision"}</button></div></form></Modal>{browser && <SourceBrowser onClose={() => setBrowser(false)} onSelect={(path) => { setDraft({ ...draft, relative_path: path }); setBrowser(false); }} />}</>;
}

function TransitionModal({ version, action, onClose, onSaved }: { version: DocumentVersion; action: string; onClose: () => void; onSaved: () => void }) {
  const [reason, setReason] = useState(""); const [password, setPassword] = useState(""); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  const signedAction = ["APPROVE", "RELEASE", "OBSOLETE"].includes(action);
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { await api(`/document-versions/${version.id}/transition`, { method: "POST", body: JSON.stringify({ action, reason, password: signedAction ? password : null }) }); onSaved(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Transition failed"); } finally { setBusy(false); } };
  return <Modal title={`${action.replaceAll("_", " ")} version ${version.version_label}`} onClose={onClose}><form onSubmit={submit} className="stack-form"><div className="signature-notice"><ShieldCheck /><p>This controlled action is permanently recorded against the exact file hash.</p></div><label>Reason<textarea value={reason} onChange={(e) => setReason(e.target.value)} required /></label>{signedAction && <label>Your password<input type="password" autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} required /></label>}<ErrorBanner error={error} /><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? "Applying…" : "Confirm action"}</button></div></form></Modal>;
}
...
function ControlledCopiesModal({ version, code, onClose }: { version: DocumentVersion; code: string; onClose: () => void }) {
  ...
  useEffect(() => { void load().catch((caught) => setError(caught.message)); }, [load]);
  const issue = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(""); try { await api(`/document-versions/${version.id}/controlled-copies`, { method: "POST", body: JSON.stringify({ ...draft, issued_to: draft.issued_to || null }) }); setDraft({ copy_number: "", department: "", location: "", issued_to: "", reason: "" }); await load(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to issue controlled copy"); } finally { setBusy(false); } };
  const close = async (copy: ControlledCopy, disposition: "RETURNED" | "DESTROYED") => { const reason = window.prompt(`Reason this copy was ${disposition.toLowerCase()}:`); if (!reason) return; try { await api(`/controlled-copies/${copy.id}/close`, { method: "POST", body: JSON.stringify({ disposition, reason }) }); await load(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to close controlled copy"); } };
